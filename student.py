...
class Student:
    ...
    def choose_target(self, world_time_str: str, schedule: Dict[str, str], rooms: Dict[str, Any]):
        ...
        if not self.is_traveling():
            ...
            if current_room_obj:
                if (
                    current_room_obj.room_type == "cafeteria"
                    and (
                        in_meal_window
                        or self.needs["hunger"] > RECOVERY_HUNGER_EXIT
                    )
                ):
                    self.target_room = self.current_room
                    return
                if (
                    current_room_obj.room_type == "dorm"
                    and (
                        self.needs["energy"] < RECOVERY_ENERGY_EXIT
                        or (in_sleep_window and self.needs["hunger"] < CRITICAL_HUNGER)
                    )
                ):
                    self.target_room = self.current_room
                    return
                if (
                    current_room_obj.room_type == "lounge"
                    and self.needs["stress"] > RECOVERY_STRESS_EXIT
                ):
                    self.target_room = self.current_room
                    return
                if (
                    current_room_obj.room_type == "bathroom"
                    and self.needs["hygiene"] < RECOVERY_HYGIENE_EXIT
                ):
                    self.target_room = self.current_room
                    return
        ...
        if self.needs["hunger"] >= PREEMPT_HUNGER and in_meal_window:
            cafeteria = self.find_room_by_type("cafeteria", rooms)
            if cafeteria is not None:
                self.target_room = cafeteria
                return
        if self.needs["energy"] <= PREEMPT_ENERGY:
            dorm = self.find_room_by_type("dorm", rooms)
            if dorm is not None:
        ...
        if self.needs["stress"] >= RECOVERY_STRESS_EXIT:
            lounge = self.find_room_by_type("lounge", rooms)
    ...
    def record_attendance(self, world_time_str: str, expected_room: Optional[str], *, traveling: bool = False):
        ...
        if (
            expected_room is not None
            and traveling
            and self.target_room == expected_room
        ):
